...
it("allows the user to register successfully", async () => {
  ...
  expect(
    screen.getByLabelText("mentor", { name: "available_to_mentor" }).checked
  ).toEqual(false);
  fireEvent.click(
  ...
  expect(
    screen.getByLabelText("mentee", { name: "need_mentoring" }).checked
  ).toEqual(false);
  fireEvent.click(screen.getByLabelText("mentee", { name: "need_mentoring" }), {
    target: { value: true },
  });
  expect(
    screen.getByLabelText("termsCheck", {
      name: "terms_and_conditions_checked",
    }).checked
  ).toEqual(false);
  fireEvent.click(
    screen.getByLabelText("termsCheck", { name: "terms_conditions_checked" }),
    {
      target: { value: true },
    }
  );
  ...
  expect(screen.getByTitle("response")).toHaveTextContent(
    "User was created successfully. A confirmation email has been sent via email. After confirming your email you can login."
  );
});
...
it("handles password toggle", () => {
  ...
  expect(screen.getByPlaceholderText("Confirm Password").type).toEqual(
    "password"
  );
...
it("checks empty field warning", async () => {
  ...
  fireEvent.change(
    screen.getByLabelText("Full Name :", { selector: "input" }),
    {
      target: { value: "" },
    }
  );
