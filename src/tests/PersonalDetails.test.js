...
it('should display personal details returned from get call', async () => {
  render(<AuthContext.Provider value={{access_token: "test_access_token"}}><PersonalDetails/></AuthContext.Provider>);
  ...
  await wait(() => {
    ...
    expect(screen.getByLabelText("Social Media Links :", {selector: "input"})).toHaveValue("http://linkedin.com/xyz");
    ...
    expect(screen.getByLabelText("Current Organization :", {selector: "input"})).toHaveValue("University");
    expect(screen.getByLabelText("Skills :", {selector: "input"})).toHaveValue("Tech, Open Source");
...
it('should send modified fields in payload to update', async () => {
  ...
  await wait(() => {
    fireEvent.change(screen.getByLabelText("Name :", {selector: "input"}), {target: {value: "Mr. XYZ"}});
    fireEvent.change(screen.getByLabelText("Occupation :", {selector: "input"}), {target: {value: "Developer"}});
