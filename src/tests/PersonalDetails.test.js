...
it("should display personal details returned from get call", async () => {
  ...
  await wait(() => {
    ...
    expect(
      screen.getByLabelText("IRC ID :", { selector: "input" })
    ).toHaveValue("xyz-new");
    expect(
      screen.getByLabelText("Social Media Links :", { selector: "input" })
    ).toHaveValue("http://linkedin.com/xyz");
    expect(
      screen.getByLabelText("mentor", { name: "available_to_mentor" })
    ).toBeChecked();
    expect(
      screen.getByLabelText("mentee", { name: "need_mentoring" })
    ).not.toBeChecked();
    expect(
      screen.getByLabelText("Interests :", { selector: "input" })
    ).toHaveValue("coding");
    expect(screen.getByLabelText("Bio :", { selector: "input" })).toHaveValue(
      "Student"
    );
    expect(
      screen.getByLabelText("Occupation :", { selector: "input" })
    ).toHaveValue("Student");
    expect(
      screen.getByLabelText("Location :", { selector: "input" })
    ).toHaveValue("India");
    expect(
      screen.getByLabelText("Current Organization :", { selector: "input" })
    ).toHaveValue("University");
    expect(
      screen.getByLabelText("Skills :", { selector: "input" })
    ).toHaveValue("Tech, Open Source");
    expect(
      screen.getByLabelText("Photo Url :", { selector: "input" })
    ).toBeEmpty();
    expect(
      screen.getByLabelText("Resume Url :", { selector: "input" })
    ).toBeEmpty();
  });
});
