...
it('handles server down', async () => {
  server.use(
    rest.post(`${BASE_API}/login`, (req, res, ctx) => {
      ...
      return res(
        ctx.status(500),
        ctx.json({ message: 'The server is currently unavailable. Try again later' })
      );
    })
  );
  ...
  expect(screen.getByLabelText('errorMessage')).toHaveTextContent(
...
it('checks if the fields are empty', async () => {
  ...
  fireEvent.change(screen.getByLabelText('Username or Email:', { selector: 'input' }), {
    target: { value: '' }
  });
