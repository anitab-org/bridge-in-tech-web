        ...
        <div className={!isAuth ? "hidden" : ""}>
            <div className="wrapper">
                <nav id="sidebar" className={isNotActive ? "active" : ""}>
                    <button type="button" id="sidebarCollapse" onClick={() => setNotActive(!isNotActive)} className="btn btn-custom">
                        <span className={isNotActive ? "" : "hidden"}>{barsIcon}</span>
                        <span className={isNotActive ? "hidden" : ""}>{crossIcon}</span>
                    </button>
                    <div className="sidebar-header">
                        <img alt="user_logo" src={image} className="rounded-circle usr-image" height={isNotActive ? "20" : "70"} width={isNotActive ? "20" : "70"}></img>
                        <h3>{user}</h3>
                    </div>
                    ...
                    <ul className="list-unstyled components">
                        ...
                        <li className="list-item">
                            ...
                            <OverlayTrigger key="bottom" placement="bottom" overlay={<Tooltip id={`tooltip-$'bottom'`}>{!isDropdownActive ? "Close My Space" : "Open My Space"}</Tooltip>}>
                                <Link to="/portfolio" href="#homeSubmenu" data-toggle="collapse" aria-expanded="false" className="dropdown-toggle" onClick={() => setDropdownActive(!isDropdownActive)}>
                                    My Space
                                </Link>
                            </OverlayTrigger>
                            <ul className={isDropdownActive ? "list-unstyled  collapse" : "list-unstyled"} id="homeSubmenu">
